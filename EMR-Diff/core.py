    ...
    for i in range(shape):
        for j in range(shape):
            ...
            if (abs(x) < a) and (abs(y) < a):
                x_denom = numpy.pi * x / a if x != 0 else 1
                y_denom = numpy.pi * y / a if y != 0 else 1
                out[i, j] = numpy.sinc(x / a) * numpy.sinc(y / a) / (x_denom * y_denom)
            else:
                out[i, j] = 0.0
    # Normalize the kernel
    return numpy.float32(out / numpy.sum(out[out != 0]))
def resize_1d(
        x: torch.Tensor,
        dim: int,
