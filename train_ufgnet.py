"""Train/evaluate UFGNet under the repository's 4x Gaussian+Bicubic + SRF protocol."""
...
@torch.no_grad()
def evaluate(model, degradation, loader, device, scale_ratio: int) -> Dict[str, float]:
    model.eval()
...
def main() -> None:
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        # Full-reference metrics are monitoring only; they never enter the loss.
        ...
        line += f" | {time.time() - epoch_start:.1f}s"
        print(line)
