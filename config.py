...
class TrainConfig:
    ...
    # UFGNet architecture. r=5 is paper-reported for Pavia.
    # Eq. (20) explicitly defines K as the total number of sampling elements and
    # gives K=9 for a 3x3 kernel, whereas the sensitivity section later calls K a
    # kernel size and reports K=7. Because that overloaded K cannot uniquely define
    # a standard DConv geometry, we keep the explicitly illustrated 3x3 DConv here.
    # The paper also omits numerical QIEM lambda and FASA tau values.
    ufg_rank: int = 5
